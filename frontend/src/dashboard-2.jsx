...
import TrackingChartComponent from "./record_graph";
...
export default function Dashboard2(props) {
  ...
  return (
    <Box
      sx={{
        // Responsive padding: smaller on mobile (xs), larger on desktop (md)
        p: { xs: 2, md: 3 },
        pb: 8,
        minHeight: "100vh",
        bgcolor: RM_COLORS.bg,
    ...
    >
      ...
      {/* Ensure these custom components handle mobile widths internally too */}
      ...
      {accounts.map((acc) => (
        <Paper
          ...
          sx={{
            mb: { xs: 3, md: 4 },
            p: { xs: 2, sm: 3 }, // Less padding on small screens
            borderRadius: { xs: 4, sm: 6 }, // Slightly less rounded on mobile
        ...
        >
          ...
          <Grid container spacing={3}>
            ...
            {/* FIX: Buttons inside a Grid container must be wrapped in a Grid item 
              to prevent layout breakages on mobile devices.
            */}
            <Grid item xs={12} sx={{ display: "flex", gap: 2, mt: 1 }}>
              <Button
                ...
                sx={{
                  flex: 1, // Makes the button take up equal space on mobile
                  color: "white",
              ...
              <Switch
                checked={!!acc.graph} // Ensure it's treated as a boolean
                onChange={() => {
                ...
                inputProps={{
                  "aria-label": `Toggle graph for ${acc.institution}`,
                }}
              />
            </Grid>
            {acc.graph && (
              <TrackingChartComponent url={props.url} accountId={acc.id} />
            )}
          </Grid>
        </Paper>
      ))}
